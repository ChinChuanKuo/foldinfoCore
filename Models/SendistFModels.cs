...
namespace FOLDINFOCore.Models
{
    public class SendistFClass
    {
        public itemModels GetSearchModels(userData userData, string cuurip)
        {
            ...
            mainRows = database.checkSelectSql("mssql", "flyfnstring", "exec web.searchinsignform @inoper;", dbparamlist);
            ...
            foreach (DataRow dr in mainRows.Rows)
            {
                ...
                items.Add(new Dictionary<string, object>() { { "iid", dr["iid"].ToString().TrimEnd() }, { "group", dr["department"].ToString().TrimEnd() }, { "showGroupMenu", false }, { "groupitems", groupitems.ToArray() }, { "id", dr["newid"].ToString().TrimEnd() }, { "name", dr["username"].ToString().TrimEnd() }, { "showNameMenu", false }, { "nameitems", nameitems.ToArray() }, { "itemModify", false } });
            }
        ...
        public statusModels GetInsertModels(iItemsData iItemsData, string cuurip)
        {
            ...
            foreach (var item in iItemsData.items)
            {
                ...
                if (database.checkActiveSql("mssql", "flyfnstring", "exec web.modifyinsignform @newid,@username,@department,@iid,@inoper;", dbparamlist) != "istrue")
                {
