...
namespace foldinfoCore.Models
{
    public class MailistIClass
    {
        public sArrayModels GetSearchModels(userData userData, string cuurip)
        {
            ...
            mainRows = new database().checkSelectSql("mssql", "flyfnstring", "exec web.searchismailmentform @inoper;", dbparamlist);
        ...
        public sRowsModels GetSItemModels(otherData otherData, string cuurip)
        {
            ...
            foreach (DataRow dr in new database().checkSelectSql("mssql", "flyfnstring", "exec web.searchismailuserform @department,@inoper;", dbparamlist).Rows)
            {
        ...
        public statusModels GetInsertModels(iFormData iFormData, string cuurip)
        {
            ...
            switch (database.checkSelectSql("mssql", "flyfnstring", "exec web.searchismailform @formId,@inoper;", dbparamlist).Rows.Count)
            {
                case 0:
            ...
            foreach (var item in iFormData.items)
            {
                ...
                dbparamlist.Add(new dbparam("@inoper", iFormData.newid.TrimEnd()));
                if (database.checkActiveSql("mssql", "flyfnstring", "exec web.updateismailform @newid,@notice,@inoper;", dbparamlist) != "istrue")
                {
