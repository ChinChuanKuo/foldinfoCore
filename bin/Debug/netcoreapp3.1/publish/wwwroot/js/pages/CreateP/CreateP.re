...
let make = _ => {
  ...
  let insertAJax = () =>
    Js.Promise.(
      ...
      |> Axiosapi.CreateP.insert
      |> then_(response =>
           {
             switch (response##data##status) {
             | "istrue" =>
               "saveSuccess" |> Sessions.create("form");
               productPath |> ReasonReactRouter.push;
             | _ =>
               response##data##status |> statusModule |> barShowRestoreAction;
               ActionShowProgress |> dispatch;
             };
                ...
                <>
                  ...
                  <GridItem top="0" right="0" bottom="0" left="0" xs="auto">
                    <TextFieldStandard
                      ...
                      disabled={state.showProgress}
                      onChange={event =>
                        i |> changeMomo(ReactEvent.Form.target(event)##value)
                      }>
                      {{js|機種|js} |> string}
                    </TextFieldStandard>
                  </GridItem>
